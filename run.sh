#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# run.sh — Lance Ch-blutter (Docker ou natif Termux/Linux)
#
# Usage :
#   ./run.sh <indir> <outdir> [options blutter...]
#
# Arguments :
#   indir    APK ou dossier contenant les .so cibles
...
#   --no-compressed-ptrs   Désactive la compression des pointeurs
#   --dart-version V       Version Dart manuelle (ex: 3.4.2_android_arm64)
#
# Options de ce script :
#   --native           Force le mode natif (python blutter.py)
#   --docker           Force le mode Docker
#   --image NAME       Nom de l'image Docker  [défaut: chblutter:latest]
...
DEFAULT_IMAGE="chblutter:latest"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --rebuild|--no-analysis|--ida-fcn|--no-update|--debug|--auto|--no-compressed-ptrs)
                           BLUTTER_ARGS+=("$1"); shift ;;
        -*)
            warn "Option inconnue : $1 (transmise à blutter)"
            BLUTTER_ARGS+=("$1")
            shift
            ;;
        *)
            ...
            else err "Argument inattendu : $1 (usage : ./run.sh <indir> <outdir> [options])"; fi
            shift
            ;;
    esac
done
...
if   [[ "${FORCE_NATIVE}" -eq 1 ]]; then
    ...
    RUN_MODE="native"
    info "Termux détecté → mode natif"
elif _has_docker; then
    RUN_MODE="docker"
    info "Docker détecté → mode Docker"
elif _has_python; then
    RUN_MODE="native"
    info "Python détecté → mode natif"
else
    err "Ni Docker ni Python disponibles.\n"\
        "  Sur Termux : ./setup_termux.sh\n"\
        "  Sur Linux  : voir README.md"
fi
...
if [[ "${RUN_MODE}" == "native" ]]; then
    ...
    CMD=("${PY}" "${BLUTTER_PY}" "${INDIR}" "${OUTDIR}" --auto)
    ...
    if [[ "${DRY_RUN}" -eq 1 ]]; then
        info "Commande (dry-run) :"
        echo -e "  ${C_DIM}${CMD[*]}${C_R}"
        exit 0
    fi
    ...
    if "${CMD[@]}"; then
        echo
        ok "Terminé — résultats : ${OUTDIR}"
    else
        echo
        err "Analyse échouée. Consultez les logs ci-dessus."
    fi
    exit 0
fi
...
if [[ "${RUN_MODE}" == "docker" ]]; then
    command -v docker &>/dev/null ||
        err "Docker introuvable : https://docs.docker.com/get-docker/"
    docker info &>/dev/null 2>&1 ||
        err "Démon Docker non démarré : sudo systemctl start docker"

    # Build image si demandé ou absente
    ...
    # Montages
    if [[ -f "${INDIR}" ]]; then
        INPUT_VOL="${INDIR}:/app/input/$(basename "${INDIR}"):ro"
        CONTAINER_INPUT="/app/input/$(basename "${INDIR}")"
    else
        INPUT_VOL="${INDIR}:/app/input:ro"
    ...
    if "${DOCKER_CMD[@]}"; then
        ...
        echo
        err "Analyse échouée (Docker). Consultez les logs ci-dessus."
    fi
    exit 0
fi
